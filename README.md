![DsMMO](./pics/modicon.png)

----------------------------

Install through the [Steam Workshop](https://steamcommunity.com/sharedfiles/filedetails/?id=942328680)


Inspired by McMMO for Minecraft, the purpose of this mod is to reward players for every action they do and make the game less grindy the longer they play (and survive).
You will get experience for several actions and over time get level-ups that let you learn new skills, rituals and increase the effect rate and chance of skills and rituals that you have already learned.
But be careful. Every time you die, you loose half of your experience.



The lore says that all skills and rituals have a 50 percent better chance to be successful during a full moon...


Table of contents
=================

* [Rituals](#rituals)
* [Skills](#skills)
* [Skill- and ritual-tree](#skill--and-ritual-tree)
  * [CHOP](#chop)
  * [MINE](#mine)
  * [ATTACK](#attack)
  * [PLANT](#plant)
  * [DIG](#dig)
  * [EAT](#eat)
  * [PICK](#pick)
  * [BUILD](#build)
* [User-commands](#user-commands)
* [Admin-commands](#admin-commands)


Rituals
=======

Rituals are a way of sacrificing items to build rare structures or performing useful actions.

Conducting a ritual
-------------------

* Place all the needed items on the ground (**but don't use stacked items or you will loose them**)
* They don't need to be placed in any specific order or arrangement but there is always a middle piece that needs to be near to all the other ingredients.
* When you have laid out all the needed items, **inspect** the middle piece (you may have to **force inspect**. You can set its key command in the settings).


Skills
======

Skills give you bonuses when you conduct an action.
The higher your level in the matching DsMMO-action, the higher is your chance and / or effectiveness of the skill.



Skill- and ritual-tree
======================


CHOP
----

The higher your CHOP-level, the faster you will be able to chop trees


MINE
----

The higher your MINE-level, the faster you will be able to mine stones


ATTACK
------

You get experience for: Attacking other player, NPCs or structures and when you are attacked.

#### Explosive touch (level 1)
Every attack you deal has a chance of doing extra damage (same damage like of a spear).

#### Beetaliation (level 2)
Every time you are attacked, there is a chance that you spawn a bee, that will attack the source that attacked you.


PLANT
-----

#### Double the shit (level 1)
Fertilizing has a chance of being extra effective.

#### Plant another day (level 2)
Harvesting crops has a chance of getting double drops. Possible drops are:
With a 50 percent chance: lightbulb, red gem, blue gem
With a 30 percent chance: mandrake, purple gem, cutreeds, slurper pelt, furtuft
With a 20 percent chance: moonrocks, beardhair, yellow gem, orange gem

#### Ritual of redness (level 3)
![Berries, Fireflies, Rot x2](./pics/recipes/berries.png)
![>](./pics/arrow_r.png)
![Berry Bush](./pics/recipes/berries_outcome.png)

#### Ritual of red juiciness (level 4)
![Juicy Berries, Fireflies, Rot x2](./pics/recipes/berries_juicy.png)
![>](./pics/arrow_r.png)
![Juicy Berry Bush](./pics/recipes/berries_juicy_outcome.png)

#### Ritual of bananana (level 6)
Can only be performed in the caves.

![Banana, Fireflies, Rot x2](./pics/recipes/cave_banana.png)
![>](./pics/arrow_r.png)
![Banana Tree](./pics/recipes/cave_banana_outcome.png)

#### Ritual of magic mushrooms (level 8)
Creates a mushroom with random color.

![Living Log, Red Cap, Green Cap, Blue Cap, Fireflies, Mushroom](./pics/recipes/living_log.png)
![>](./pics/arrow_r.png)
![Mushroom (random color)](./pics/recipes/living_log_outcome.png)


DIG
---

#### Treasure hunter (level 1)
Using your shovel for digging has a chance of spawning additional items

#### Ritual of mole infestation (level 2)
![Burrow, 3 different ores](./pics/recipes/molehill.png)
![>](./pics/arrow_r.png)
![Spawns a Mole (or three, if lucky)](./pics/recipes/molehill_outcome.png)

#### Ritual of mole attraction (level 3)
![Shovel, Hounds Tooth, Log](./pics/recipes/shovel.png)
![>](./pics/arrow_r.png)
![Teleports all nearby moles to the current position](./pics/recipes/shovel_outcome.png)

#### Ritual of roman streets (level 5)
![Pitchfork, Stones x3, Boards x3, Guano x3](./pics/recipes/pitchfork.png)
![>](./pics/arrow_r.png)
![Creates three cobblestone-tiles](./pics/recipes/pitchfork_outcome.png)


EAT
---

#### Self-cannibalism (level 3)
Exchange DsMMO-experience to completely fill your health, hunger or sanity.
Use this skill with the following chat-command:
```
#dsmmo eat [action] [health | hunger | sanity]
```
The higher your EAT-action-level the less DsMMO-experience is used for healing yourself.

#### Ritual of death (level 5)
![Amulet, Mandrake x2, Nightmare Fuel x2](./pics/recipes/amulet.png)
![>](./pics/arrow_r.png)
![Death without loosing DsMMO-exp (but EAT-exp will be set to 0)](./pics/recipes/amulet_outcome.png)

#### Ritual of a new life (level 8)
![Deerclops Eyeball, Thick Fur x2. Fireflies x2, Down Feather, Scales](./pics/recipes/deerclops_eyeball.png)
![>](./pics/arrow_r.png)
![Change player](./pics/recipes/deerclops_eyeball_outcome.png)


PICK
----

#### Ghosty fireflies (level 1)
As a ghost, haunting evil flowers has a chance to turn into fireflies. The higher your PICK-action-level, the higher your chance of succeeding.

#### Ritual of the longest Twig (level 2)
![Twigs, Log x2, Fireflies](./pics/recipes/twigs.png)
![>](./pics/arrow_r.png)
![Sapling](./pics/recipes/twigs_outcome.png)

#### Ritual of reggae dreams (level 3)
![Cut Grass, Rot x2, Fireflies](./pics/recipes/cutgrass.png)
![>](./pics/arrow_r.png)
![Grass](./pics/recipes/cutgrass_outcome.png)

#### Ritual of shiny balls (level 5)
Can only be performed in the caves.

![Lightbulb, Twigs x2, Fireflies](./pics/recipes/lightbulb.png)
![>](./pics/arrow_r.png)
![Light Bulb](./pics/recipes/lightbulb_outcome.png)

#### Ritual of Poe (level 7)
Can only be performed on marsh-ground.

![Cut reeds, Rot x2, Fireflies](./pics/recipes/cutreeds.png)
![>](./pics/arrow_r.png)
![Reeds](./pics/recipes/cutreeds_outcome.png)


BUILD
-----

#### Ritual of pussy love (level 1)
![Cat Tail, Meat, Hounds Tooth x2, Fireflies x2](./pics/recipes/coontail.png)
![>](./pics/arrow_r.png)
![Hollow Stump](./pics/recipes/coontail_outcome.png)

#### Ritual of splishy splashy (level 2)
The resulting pond type depends on the ground and place (forest / cave, Marsh-ground / other) where it is summoned.

![Fish, Frog Legs, Mosquito Sack, Fireflies x2](./pics/recipes/fish.png)
![>](./pics/arrow_r.png)
![Pond](./pics/recipes/fish_outcome.png)

#### Ritual of dumb monkeys (level 3)
Can only be performed in the caves.

![Cooked Banana, Purple Gem, Manure x2, Fireflies x2](./pics/recipes/cave_banana_cooked.png)
![>](./pics/arrow_r.png)
![Splumonkey Pod](./pics/recipes/cave_banana_cooked_outcome.png)

#### Ritual of arctic fishing (level 4)
Removes an existing walrus camp.

![Walrus Camp, Tam o' Shanter x2, Walking Cane x2, Fireflies x2](./pics/recipes/walrus_camp.png)
![>](./pics/arrow_r.png)
![Removed Walrus Camp](./pics/recipes/walrus_camp_outcome.png)

#### Ritual of whalers feast (level 5)
![Walrus Tusk, Tam o' Shanter x2, Walking Cane x2, Fireflies x2](./pics/recipes/walrus_tusk.png)
![>](./pics/arrow_r.png)
![Walrus Camp](./pics/recipes/walrus_tusk_outcome.png)

#### Ritual of Saurons bird (level 6)
![Tallbird Egg, Beard Hair x2, Cut Grass x2, Fireflies x2](./pics/recipes/tallbirdegg.png)
![>](./pics/arrow_r.png)
![Tallbird Nest](./pics/recipes/tallbirdegg_outcome.png)

#### Ritual of the pigable flame (level 7)
![Firepit, Pig Skin x4, Scale, Fireflies x2](./pics/recipes/firepit.png)
![>](./pics/arrow_r.png)
![Pig Torch](./pics/recipes/firepit_outcome.png)

#### Ritual of rerevival (level 8)
(only works with player skeletons)

![Player Skeleton, Nightmare Fuel, Marble x2, Amulet](./pics/recipes/skeleton_player.png)
![>](./pics/arrow_r.png)
![Makes on random used touchstone useable again](./pics/recipes/skeleton_player_outcome.png)

#### Ritual of homing flame (level 10)
Costs one Build-level!
Does not teleport to campfires.

![Campfire, Log x2, Pine Cones x2, Cut Grass x2, Twigs x2](./pics/recipes/campfire.png)
![>](./pics/arrow_r.png)
![Teleport to last used firepit](./pics/recipes/campfire_outcome.png)



User-commands
=============

User-commands are available for all players. To use them they have to **whispered** into the chat.


```#dsmmo [action]```
---------------------

This command spawns a mini-sign at your current position (which will despawn over time). The sign always shows your last used DsMMO-action and if you examine it, you will be shown your current level, experience and learned skills.
If you specify an action, the mini-sign will show the specified action instead of the last used one.


```#dsmmo list```
-----------------

Lists the levels of all your DsMMO-actions


```#dsmmo eat [action] [hunger | health | sanity]```
----------------------------------------------------

See the skill [Self-cannibalism](#self-cannibalism-level-3)



Admin-commands
==============

Admin-commands are only available if you have remote access to the server-console.


```dsmmo_reset(player_name)```
------------------------------

Resets all DsMMO-actions to level 0 for the specified player.
The target player has to be online for the command to work properly.


```dsmmo_set(player_name, action, level)```
-------------------------------------------

Sets the level of a specific DsMMO-action to the specified level for the specified player.
The target player has to be online for the command to work properly.



This is a server-only mod which means no fancy UI but other users don't need to download anything as long as the server-admin has this mod activated